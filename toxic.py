...
with open("./important_features.json","r") as fp:
	target_words = json.load(fp)
...
def predict_message(msg):
	...
	for word in words:
		if word in target_words:
			words_in_target.append(words)

	if len(words_in_target) > 0:
		is_toxic = True
	...
	for target, pattern in patterns.items():
		for pat in pattern:
			...
			if len(re.findall(pat,clean_msg)) > 1:
				is_toxic = True	
	return is_toxic
...
if __name__ == "__main__":
	msg = sys.argv[1]
	flag = predict_message(msg)
	if flag:
		print("Message is toxic")
	else:
		print("Message is not toxic")